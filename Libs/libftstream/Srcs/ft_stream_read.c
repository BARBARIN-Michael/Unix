...
void		ft_stream_read_full(t_stream *stream)
{
	...
	stream->allread += cursor_read;
	stream->cursor = 0;
	stream->size = 0;
	if (ft_stream_fill(stream) <= 0)
		return ;
	ft_stream_read_full(stream);
}
...
unsigned int		ft_stream_read_n(t_stream *stream, int n)
{
	int		cursor_read;
